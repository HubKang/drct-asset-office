...
def test_daily_return_falls_back_to_saved_price_and_flow_when_theme_snapshot_is_missing() -> None:
    ...
    with engine.begin() as conn:
        ...
        conn.exec_driver_sql("CREATE TABLE market_theme_stocks (id INTEGER PRIMARY KEY, theme_id INTEGER, stock_id INTEGER, is_active INTEGER, is_primary INTEGER, stock_memo TEXT)")
        ...
        conn.exec_driver_sql("INSERT INTO market_theme_stocks VALUES (100, 1, 10, 1, 1, '변압기')")
    ...
    assert result.stocks[0].mapping_id == 100
...
def test_latest_return_lists_linked_stocks_before_first_return_refresh() -> None:
    ...
    with engine.begin() as conn:
        ...
        conn.exec_driver_sql("INSERT INTO market_theme_stocks VALUES (100, 1, 10, 1, 1, '전력기기'), (200, 1, 20, 1, 0, '정유·ESS')")
    ...
    assert [stock.mapping_id for stock in result.stocks] == [100, 200]
