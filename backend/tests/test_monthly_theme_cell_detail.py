...
def test_monthly_theme_cell_detail_uses_saved_event_stocks_and_selected_date_flow(monkeypatch) -> None:
    ...
    assert [stock.stock_code for stock in detail.stocks] == ["000001", "000002"]
    assert detail.stocks[0].flow_summary is not None
    assert detail.stocks[0].flow_summary.individual_net_amount == -1_000_000_000
    assert detail.stocks[1].flow_summary is None
