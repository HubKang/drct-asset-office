...
class PatternResearchService:
    ...
    def gpt_goal_parse_prompt(self, goal_text: str, parsed_goal: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        prompt = (
            ...
            "DrCT 1차 해석은 후보입니다. 사용자의 자연어 의도와 맞는지 검증하고, 필요한 경우 더 적절한 수식 또는 신규 계산식 후보를 제안하세요.\n"
            "1단계에서 선택된 GPT 검증 대상 수식과 수식화 필요 문장은 모두 검토 대상입니다. 성공/실패 기준은 분석 전제로 참고하세요.\n"
...
[연구 목적]
이 연구의 목적은 특정 종목을 추천받거나 매수·매도 판단을 받는 것이 아닙니다.

목적은 DrCT가 추출한 성공 샘플과 실패 샘플을 비교하여,
성공 확률이 높은 매매패턴의 구조를 찾아내는 것입니다.

특히 다음과 같은 형태의 핵심 통찰을 원합니다.
- 성공 샘플과 실패 샘플의 가장 큰 차이는 이격 과열입니다.
- 실패 샘플은 20일선/60일선 대비 이격이 큰 상태에서 진입한 경우가 많습니다.
- 성공 샘플은 단순 거래대금 증가보다 과열 해소 후 재상승 구간에서 더 많이 발생합니다.
- 5일선과 10일선의 근접 자체보다, 최근 5일 수익률이 과열되지 않은 상태가 더 중요해 보입니다.

사용자가 처음 제시한 조건만 평가하지 말고,
사용자가 포함하지 않았지만 데이터상 성공/실패를 구분하는 데 유의미해 보이는 조건 후보도 찾아 주세요.

제안하는 조건 후보는 DrCT에서 다시 테스트할 수 있도록 가능한 한 수식으로 정리해 주세요.

이후 사용자는 제안된 조건을 DrCT에 다시 등록하고,
조건값을 수정하며 반복 테스트하여 점점 승률 높은 복합 조건을 만들어 갈 예정입니다.

따라서 단순한 해석보다 “다음 테스트에 넣어볼 만한 조건 후보”를 구체적으로 제안해 주세요.
...
1. 현재 성공률의 의미를 평가해 주세요.
   - 단순히 높다/낮다가 아니라, 현재 샘플 적용 조건이 너무 넓은지 평가해 주세요.
2. 성공 샘플과 실패 샘플의 가장 큰 차이 3가지를 정리해 주세요.
   - 예: 이격 과열, 최근 수익률 과열, 거래대금 증가의 실효성, 이동평균 배열, 눌림 깊이 등
3. “성공 샘플과 실패 샘플의 핵심 차이는 무엇이다”라는 한 문장 통찰을 제시해 주세요.
   - 예: 핵심 차이는 20일선/60일선 대비 이격 과열 여부입니다.
4. 실패 샘플에서 반복되는 진입 실패 패턴을 찾아 주세요.
   - 과열 진입
   - 추격 진입
   - 눌림이 아닌 하락 지속
   - 거래대금 증가 착시
   - 조건 미충족 진입
5. 사용자가 자연어로 제시한 조건이 실제로 유효한지 평가해 주세요.
   - 5일선과 10일선 근접
   - 거래대금 재유입
   - 60일선 상승추세
   - 급등 직후 제외
6. 샘플 관찰 지표 중 성공/실패를 가장 잘 구분하는 지표를 찾아 주세요.
   - 사용자가 직접 조건으로 넣지 않았더라도 유의미하면 반드시 제안해 주세요.
7. 사용자가 포함하지 않았지만 추가하면 좋을 조건 후보를 최소 3개 이상 제안해 주세요.
   - 각 조건은 DrCT 조건식으로 표현해 주세요.
   - 기대 효과와 과최적화 위험도 함께 제시해 주세요.
8. 현재 조건 후보 중 샘플 적용 조건으로 승격할 만한 것이 있는지 판단해 주세요.
   - 승격 추천
   - 보류
   - 제외
   - 기준값 수정
9. 성공 가능성이 높은 복합 패턴 후보 2~3개를 제안해 주세요.
   - 각 패턴은 여러 조건의 조합으로 제시해 주세요.
   - 예: 위치 조건 + 과열 제외 조건 + 거래대금 확인 조건
10. 각 복합 패턴 후보를 DrCT 조건식으로 다시 정리해 주세요.
   - 진입 조건
   - 제외 조건
   - 성공 기준
   - 실패 기준
   - 샘플 관찰 지표
11. 다음 DrCT 테스트에 바로 넣어볼 우선순위 조건 세트를 제안해 주세요.
   - 1순위 테스트 조건
   - 2순위 테스트 조건
   - 3순위 테스트 조건
12. 매매훈련용 체크리스트를 만들어 주세요.
   - 진입 전 확인
   - 진입 금지
   - 손절 관리
   - 복기 포인트
13. 과최적화 위험과 추가 검증 필요사항을 알려 주세요.
   - 샘플 수 부족
   - 특정 종목 편향
   - 특정 급등 구간 편향
   - 조건이 너무 많은 경우의 위험
...
- DrCT 조건식:
- 추천 기준값:
- 적용 구분: 진입 조건 / 제외 조건 / 샘플 관찰 지표
- 기대 효과:
- 주의점:
- 샘플 수 감소 또는 과최적화 위험:
- 다음 테스트 여부: 우선 테스트 / 보류 / 제외
...
[매매훈련 연결]
최종 목적은 제안된 조건을 DrCT에서 반복 테스트한 뒤,
사용자의 매매훈련 체크리스트와 매매기법 원칙으로 연결하는 것입니다.

따라서 체크리스트는 실제 훈련 화면에서 사용할 수 있도록 짧고 구체적으로 작성해 주세요.

예:
- 20일선/60일선 이격 과열 여부를 확인했는가?
- 최근 5일 급등 후 추격 진입은 아닌가?
- 거래대금 증가는 실제 재유입인가, 단기 과열의 잔상인가?
- 손절 기준 -5% 이내에서 대응 가능한 자리인가?
"""
