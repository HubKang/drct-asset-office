...
function ResponseBar({ title, data }: { title: string; data: ScenarioResponseDistribution }) {
  ...
  return <section className="scenario-habit-section"><div className="scenario-section-head"><strong>{title}</strong><span className="scenario-basis-label">도달 구간 기준</span></div>
    {total === 0 ? <div className="scenario-no-data"><strong>{title.replace(" 대응 속도", "")} 구간 없음</strong><p>선택 기간에 해당 계획가격 도달 Episode가 없습니다.</p></div> : <>
      <p className="scenario-response-total">도달 구간 {total}건</p>
      ...
      <div className="scenario-bar-legend">{rows.map(([label, count, value, key]) => <span key={key}><i className={key} />{label} {count}건 · {pct(value)}</span>)}</div>
      {Number(data.max_unresolved_bars || 0) > 0 ? <p className="scenario-response-note">최대 미응답 지속 {data.max_unresolved_bars}봉</p> : null}
    </>}
  </section>;
}
function ExecutionTrend({ data, onSelect }: { data: ScenarioHabitsResponse; onSelect: (id?: number | null) => void }) {
  ...
  const width = Math.max(620, points.length * 70 + 80), height = 220, average = Number(data.summary.average_execution_rate || 0);
  const x = (i: number) => 45 + i * ((width - 90) / Math.max(1, points.length - 1));
  const y = (v: number) => 20 + (100 - v) * 1.55;
  return <>{points.length < 3 ? <div className="scenario-sample-note"><span>추세 참고용</span>평가 가능 거래 {points.length}건 · 거래가 더 누적되면 추세 해석의 신뢰도가 높아집니다.</div> : null}<div className="scenario-trend-scroll"><svg viewBox={"0 0 " + width + " " + height} className="scenario-trend-chart" role="img" aria-label="거래별 매매시나리오 실행률 추세">
    {[0,25,50,75,100].map((v) => <g key={v}><line x1="45" x2={width-30} y1={y(v)} y2={y(v)} className="grid"/><text x="8" y={y(v)+4}>{v}</text></g>)}
    <line x1="45" x2={width-30} y1={y(average)} y2={y(average)} className="average"/><polyline points={points.map((r,i)=>x(i)+","+y(Number(r.score))).join(" ")} className="trend"/>
    {points.map((r,i)=><g key={r.trade_sequence+"-"+i} className="scenario-trend-point" onClick={()=>onSelect(r.scenario_id)}><circle cx={x(i)} cy={y(Number(r.score))} r="6" className={r.result_type==="WIN"?"win":r.result_type==="LOSS"?"loss":"flat"}/><text x={x(i)} y="202" textAnchor="middle">#{r.trade_sequence}</text><title>{r.stock_name+" "+pct(r.score)}</title></g>)}
  </svg></div></>;
}
...
function ScenarioCategoryBars({ rows }: { rows: ScenarioCategoryScore[] }) {
  return <div className="scenario-category-list">{rows.map((row) => {
    ...
    return <details key={row.key} className={applicable ? undefined : "not-applicable"} open={applicable}>
      <summary><span>{row.label}</span><strong>{applicable ? pct(row.rate ?? row.score) : "평가 대상 없음"}</strong></summary>
      {applicable ? <>
        <Progress value={row.rate ?? row.score}/>
        <div className="scenario-category-meta"><span>평가 {tradeCount}거래 · {itemCount}항목</span><strong>{Number(row.earned_score || 0).toFixed(2)} / {Number(row.max_score || 0).toFixed(2)}점</strong></div>
        <div className="scenario-category-distribution"><i className="full" style={{flex:Number(row.full_count || 0)}}/><i className="partial" style={{flex:Number(row.partial_count || 0)}}/><i className="miss" style={{flex:Number(row.miss_count || 0)}}/></div>
        <div className="scenario-category-legend"><span>완전 실행 {row.full_count || 0}</span><span>사유 기록 {row.partial_count || 0}</span><span>미이행 {row.miss_count || 0}</span></div>
        <p>실행률 = {Number(row.earned_score || 0).toFixed(2)} ÷ {Number(row.max_score || 0).toFixed(2)} × 100 = {pct(row.rate ?? row.score)}</p>
      </> : <p>해당 거래들에는 이 영역의 평가 항목이 없습니다.</p>}
    </details>;
  })}</div>;
}
...
function ProfitLossAsymmetryPanel({ data, infoOpen, onToggleInfo }: { data: ScenarioHabitsResponse["asymmetry"]; infoOpen: boolean; onToggleInfo: () => void }) {
  ...
  const ratioMax = Math.max(plRatio, 1);
  ...
  return <section className="scenario-habit-section scenario-asymmetry-panel">
    ...
    <div className="scenario-ratio-block">
      <div><div className="scenario-ratio-title"><strong>Winning Ratio {pct(data.winning_ratio)}</strong><span>수익 {data.win_count || 0}건 · 손실 {data.loss_count || 0}건 · 보합 {data.flat_count || 0}건</span></div><Progress value={data.winning_ratio}/></div>
      <div><div className="scenario-ratio-title"><strong>Profit/Loss Ratio</strong><span>평균 수익 {plRatio.toFixed(2)} : 평균 손실 1.00</span></div>
        <div className="scenario-ratio-compare"><span>평균 수익</span><Progress value={plRatio / ratioMax * 100} tone="red"/><em>{plRatio.toFixed(2)}</em></div>
        <div className="scenario-ratio-compare"><span>평균 손실</span><Progress value={1 / ratioMax * 100}/><em>1.00</em></div>
      </div>
    </div>
  </section>;
}
export default function ScenarioHabitsPanel({ accountId, onOpenResult }: Props) {
  ...
  return <div className="scenario-habits">
    ...
    <section className="scenario-habit-overview">
      <div><span>평균 매매시나리오 실행률</span><strong>{pct(data.summary.average_execution_rate)}</strong><Progress value={data.summary.average_execution_rate}/><small>평가 가능 {evaluableCount} / 완료 {closedCount}거래</small></div>
      <div><span>계획 생성률</span><strong>{pct(scenarioCreationRate)}</strong><Progress value={scenarioCreationRate}/><small>시나리오 작성 {scenarioCreatedCount} / 완료 {scenarioDenominator}거래</small></div>
      <div><span>계획 밖 주문 비율</span><strong>{pct(data.summary.unplanned_order_rate)}</strong><Progress value={data.summary.unplanned_order_rate} tone="neutral"/><small>계획 밖 주문 {unplannedCount} / 평가 주문 {evaluatedOrderCount}건 · 낮을수록 계획 일관성이 높습니다.</small></div>
    </section>
    {evaluableCount<3?<div className="scenario-sample-note"><span>표본 적음</span>평가 거래가 적어 비율 변동이 클 수 있습니다.</div>:null}
    <section className="scenario-habit-section"><div className="scenario-section-head"><strong>거래별 매매시나리오 실행률</strong><span className="scenario-basis-label">평가 가능한 완료 거래 기준</span></div><ExecutionTrend data={data} onSelect={id=>setSelected(data.trades.find(t=>t.scenario_id===id)||null)}/></section>
    ...
    <div className="scenario-habit-split"><ResponseBar title="익절 도달 대응 속도" data={data.target_response_distribution}/><ResponseBar title="손절 도달 대응 속도" data={data.stop_response_distribution}/></div>
    <ProfitLossAsymmetryPanel data={data.asymmetry} infoOpen={infoOpen} onToggleInfo={()=>setInfoOpen(v=>!v)}/>
    <section className="scenario-habit-section"><div className="scenario-section-head"><strong>계좌 위험 사용</strong><span className="scenario-basis-label">현재 포지션 기준 · 한도 {pct(data.account_risk.max_open_risk_pct)}</span></div><div className="scenario-risk-bullet"><i style={{width:Math.min(100,Number(data.account_risk.current_open_risk_pct||0))+"%"}}/>{[60,80,100].map(v=><b key={v} style={{left:v+"%"}}/>)}</div>
      <div className="scenario-risk-positions">{data.account_risk.positions.length?data.account_risk.positions.map(r=><span key={r.session_id}>{r.stock_name} {pct(r.risk_usage_pct)}</span>):<span>진행 중인 보유 포지션 없음</span>}</div><p className="scenario-data-pending">변동성 기반 비중 분석 데이터 수집 전</p>
    </section>
    <div className="scenario-habit-split"><section className="scenario-habit-section"><div className="scenario-section-head"><strong>계획 변경</strong><span>{data.plan_change_distribution.total}건</span></div><Progress value={data.plan_change_distribution.reason_recording_rate}/><p>사유 기록률 {pct(data.plan_change_distribution.reason_recording_rate)}</p></section><section className="scenario-habit-section"><div className="scenario-section-head"><strong>계획 밖 행동</strong><span>{data.unplanned_action_distribution.total}건</span></div><Progress value={data.unplanned_action_distribution.reason_recording_rate} tone="neutral"/><p>사유 기록률 {pct(data.unplanned_action_distribution.reason_recording_rate)}</p></section></div>
    <div className="table-shell scenario-trade-table"><table className="data-table compact-table"><thead><tr><th>#</th><th>종목</th><th>청산일</th><th>손익</th><th>수익률</th><th>매매시나리오 실행률</th><th>최대 위험</th><th>계획 밖</th><th>상세</th></tr></thead><tbody>{data.trades.map(t=><tr key={t.id} onClick={()=>setSelected(t)}><td>#{t.trade_sequence}</td><td>{t.stock_name||t.stock_code}</td><td>{t.closed_chart_date}</td><td>{won(t.net_pnl)}</td><td>{pct(t.return_pct)}</td><td>{t.has_scenario_data?<><Progress value={t.scenario_execution_rate}/><small>{pct(t.scenario_execution_rate)}</small></>:"시나리오 데이터 없음"}</td><td>{pct(t.max_risk_pct)}</td><td>{t.unplanned_action_count}건</td><td><button type="button" className="btn btn-secondary">보기</button></td></tr>)}</tbody></table></div>
    {selected?<div className="scenario-drawer-backdrop" onMouseDown={()=>setSelected(null)}><aside className="scenario-review-drawer" role="dialog" aria-modal="true" onMouseDown={e=>e.stopPropagation()}><div className="scenario-section-head"><strong>거래 #{selected.trade_sequence} {selected.stock_name||selected.stock_code}</strong><button type="button" className="scenario-info-button" onClick={()=>setSelected(null)} aria-label="닫기"><X size={18}/></button></div>
      ...
      {review?<><ScenarioCategoryBars rows={review.category_scores}/><ol className="scenario-timeline">{review.timeline.map((r,i)=><li key={String(r.id||i)}><i className={String(r.severity||"INFO").toLowerCase()}/><div><strong title={String(r.event_type||"")}>{getTradeTrainingEventLabel(String(r.event_type||""))}</strong><span>{String(r.chart_date||r.created_at||"-")}</span><p>{String(r.message||"")}</p></div></li>)}</ol></>:<p>{selected.has_scenario_data?"실행 이력을 불러오는 중입니다.":"시나리오 데이터 없음"}</p>}
