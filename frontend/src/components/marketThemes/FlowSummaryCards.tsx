...
export function ThemeFlowOverview({ summary, onActorClick, highlightedActors = [], title = "테마 수급 현황", note }: { summary: ThemeDailyFlowSummary; onActorClick: (actor: FlowActor) => void; highlightedActors?: FlowActor[]; title?: string; note?: string }) {
  ...
  return <section className="theme-flow-overview">
    <div className="theme-flow-overview-head"><div><h4>{title}</h4><p>{summary.base_date} · 현재 활성 연결 종목 · 전체 반영{note ? ` · ${note}` : ""}</p></div><span className={`price-flow-quality is-${summary.quality_status.toLowerCase()}`}>수급 데이터 {summary.complete_stock_count}/{summary.connected_stock_count}종목</span></div>
