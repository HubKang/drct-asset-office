...
type Props = { activeTab: "themes" | "mapping" | "trend"; onSummaryChange: (summary: UsThemeSummary) => void };
type ViewMode = "group" | "theme";
...
const heatColor = (value: number | undefined) => value == null ? "#f8fafc" : value <= -10 ? "#60a5fa" : value < 0 ? "#dbeafe" : value >= 10 ? "#f87171" : value > 0 ? "#fee2e2" : "#f1f5f9";
...
export default function UsMarketThemesPanel({ activeTab, onSummaryChange }: Props) {
  ...
  useEffect(() => { if (activeTab === "themes" && viewMode !== "group" && viewMode !== "theme") setViewMode("theme"); }, [activeTab, viewMode]);
  ...
  return <>
    ...
    {activeTab === "trend" ? <UsThemeTrendPanel groups={groups} themes={themes} onOpenDetail={(themeId, tradeDate) => setDetailTarget({ themeId, tradeDate })} /> : activeTab === "themes" ? <SectionCard title="" className="market-theme-management-card us-theme-management-card">
      <div className="us-theme-management-header">
        ...
        <div className="us-theme-refresh-row"><span>최종 테마등락률 {themes.map((theme) => theme.latest_return_date).filter((value): value is string => Boolean(value)).sort().slice(-1)[0] || "-"}</span><button type="button" className="btn btn-danger" disabled={refreshing} onClick={() => void refreshMarket()}><RefreshCw size={14} className={refreshing ? "animate-spin" : ""}/> {refreshing ? "갱신 중..." : "미국 종가·테마 갱신"}</button></div>
      </div><div className="market-theme-filter-toolbar us-theme-filter-toolbar">
        ...
        <button type="button" className="btn btn-secondary" onClick={() => openEditor({ kind: viewMode, value: null } as EditTarget)}>+ {viewMode === "group" ? "그룹" : "테마"} 등록</button>
      </div>
