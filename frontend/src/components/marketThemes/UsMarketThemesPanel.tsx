...
function UsThemeIntegratedLineChart({ themes, dates, metric, period, onOpen }: { themes: UsThemeTrendItem[]; dates: string[]; metric: "theme_strength" | "simple_return"; period: 20 | 30 | 60; onOpen: (themeId: number, date: string) => void }) {
  ...
  return <div className="theme-return-line-panel us-theme-integrated-line" style={style}>
    ...
    <div className="theme-return-line-body"><div className="theme-return-line-chart"><svg className="theme-return-line-svg" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" role="img" aria-label={`미국 테마 ${period}거래일 30일 누적 선그래프`}>
      ...
      {series.map((item) => { const active = hoveredThemeId === item.theme_id, muted = hoveredThemeId != null && !active; const points = item.values.map((value, index) => value == null ? null : { x: xScale(index), y: yScale(value) }); const connectedPoints = points.filter((point): point is { x: number; y: number } => point != null); return <g key={item.theme_id} className={muted ? "theme-flow-line-series is-muted" : active ? "theme-flow-line-series is-active" : "theme-flow-line-series"}>
        {connectedPoints.length > 1 ? <path className="theme-return-line-path" d={linePath(connectedPoints)} fill="none" stroke={item.color} onMouseEnter={() => setHoveredThemeId(item.theme_id)} onMouseLeave={() => setHoveredThemeId(null)}><title>{`${item.theme_name}\n30일 누적 ${pct(item.cumulative)}\n최근 일별 ${pct(item.latestDaily)}`}</title></path> : null}
...
export default function UsMarketThemesPanel({ activeTab, onSummaryChange }: Props) {
  ...
  const refreshGroup = async (group: UsThemeGroup) => {
    if (!window.confirm(`${group.name} 그룹의 활성 테마 연결 종목 가격과 테마등락률을 갱신할까요?`)) return;
    setRefreshingGroupId(group.id); setError(""); setMessage("");
    try { const result = await repositories.usMarketThemes.refreshGroup(group.id); setMessage(result.message); await loadBase(); }
    catch (reason) { setError(errorText(reason, `${group.name} 테마그룹을 갱신하지 못했습니다.`)); }
    finally { setRefreshingGroupId(null); }
  };
  const refreshTheme = async (theme: UsTheme) => {
    if (!window.confirm(`${theme.name} 테마의 활성 연결 종목 가격과 테마등락률을 갱신할까요?`)) return;
    setRefreshingThemeId(theme.id); setError(""); setMessage("");
    try { const result = await repositories.usMarketThemes.refreshTheme(theme.id); setMessage(result.message); await loadBase(); }
    catch (reason) { setError(errorText(reason, `${theme.name} 테마를 갱신하지 못했습니다.`)); }
    finally { setRefreshingThemeId(null); }
  };
