...
import { AlertCircle, BarChart3, Check, ChevronRight, Database, Info, Link2, Radar, RefreshCw, SearchX, X } from "lucide-react";
...
const koreanLabel = (value: string) => ({ S: "성공", F: "실패", SUCCESS: "성공", FAILURE: "실패", UNDECIDED: "미판정", CONFLICT: "판정 충돌" }[value] ?? value);
...
function SignalAnalysisWorkspace({ detail, onManageMarkers, onError }: Props) {
  ...
  return <section className="drct-analysis-workspace" aria-labelledby="drct-analysis-title">
    <header className="drct-analysis-header"><div><h3 id="drct-analysis-title">선택 검색식 분석 &amp; 검증</h3><p>현재 탐지와 관련 Marker D0에서의 검색조건 차이를 참고용으로 검증합니다.</p></div><button type="button" className="btn btn-secondary" disabled={busy} onClick={() => void refresh()}><RefreshCw size={14} /> 데이터 새로고침</button></header>
    <nav className="drct-analysis-tabs" aria-label="선택 검색식 분석 메뉴">{([['detect','현재 탐지'],['readiness','검색식 검증'],['validation','검증 결과'],['cases','사례 분석']] as const).map(([key,label]) => <button type="button" key={key} className={tab === key ? "is-active" : ""} onClick={() => setTab(key)}>{label}{key === "cases" && summary ? <span>{summary.rule_no_match_count + summary.rule_data_incomplete_count + summary.undecided_count}</span> : null}</button>)}</nav>
    ...
    {tab === "readiness" ? <div className="drct-analysis-panel"><div className="drct-learning-status"><div><span>학습 상태</span><strong>{ruleMatch === 0 && (summary?.rule_evaluable_count ?? 0) > 0 ? "검색조건 검토 필요" : summary?.blocking_reasons.length ? "준비 항목 확인 필요" : "학습 데이터 준비"}</strong><p>{ruleMatch === 0 && (summary?.rule_evaluable_count ?? 0) > 0 ? `과거 평가 가능 사례 ${summary?.rule_evaluable_count}건 중 현재 DrCT 검색조건과 일치한 사례가 없습니다.` : "학습 단계별 데이터 준비 상태를 확인합니다."}</p></div>{(summary?.rule_no_match_count ?? 0) > 0 ? <button type="button" className="btn btn-primary" onClick={goMismatch}><SearchX size={15} /> 불일치 원인 확인</button> : null}</div>
      <section className="drct-learning-markers"><div><h4>관련 차트마커 <span>{detail.marker_links.length}개</span></h4><p>향후 검색식 Reference Factor 비교에 사용할 참고 관계이며 Marker 학습의 필수조건이 아닙니다.</p></div><button type="button" className="btn btn-secondary" onClick={onManageMarkers}><Link2 size={14} /> 관련 마커 관리</button><div>{detail.marker_links.map((link) => <span key={link.id}>{link.marker_symbol} {link.marker_name}</span>)}</div></section>
    ...
    {tab === "cases" ? <div className="drct-analysis-panel"><div className="drct-case-categories">{([['mismatch',`검색조건 불일치 ${summary?.rule_no_match_count ?? 0}`],['incomplete',`데이터 부족 ${summary?.rule_data_incomplete_count ?? 0}`],['undecided',`미판정 ${summary?.undecided_count ?? 0}`],['model',`모델 불일치 ${report?.model_disagreement_cases.length ?? 0}`]] as const).map(([key,label]) => <button type="button" key={key} disabled={key === "model" && !report?.model_disagreement_cases.length} className={category === key ? "is-active" : ""} onClick={() => { setCategory(key); setSelectedCondition(null); }}>{label}</button>)}</div>
      {category === "mismatch" && mismatch ? <><div className="drct-tab-title"><div><h4>검색조건 불일치 <span>{mismatch.case_count}건</span></h4><p>Marker D0에서 검색조건을 재평가한 결과입니다. 조건 실패 빈도가 최종 탈락의 단독 원인이라는 의미는 아닙니다.</p></div></div><div className="drct-mismatch-grid"><section><h5>어떤 조건이 많이 실패하는가?</h5><div className="drct-mismatch-table"><table><thead><tr><th>조건</th><th>검색조건</th><th>충족</th><th>미충족</th><th>부족</th><th>실패율</th></tr></thead><tbody>{mismatch.conditions.map((item) => <tr key={item.code} className={selectedCondition === item.code ? "is-selected" : ""} onClick={() => setSelectedCondition(item.code)}><td><b>{item.code}</b></td><td title={item.label}>{item.label}</td><td className="is-pass">{item.pass_count}</td><td className="is-fail">{item.fail_count}</td><td>{item.incomplete_count}</td><td><div className="drct-fail-rate"><strong>{pct(item.fail_rate)}</strong><i><span style={{width:`${Math.min(100,item.fail_rate ?? 0)}%`}} /></i></div></td></tr>)}</tbody></table></div>{mismatch.branches.length ? <div className="drct-branch-summary"><h5>조정 유형별 통과 현황</h5><div>{mismatch.branches.map((item, index) => <article key={item.expression}><span>{index === 0 ? "5일선 조정" : index === 1 ? "10일선 조정" : "20일선 조정"}<small>{item.label}</small></span><strong>{item.pass_count} / {item.evaluated_count}</strong></article>)}</div></div> : null}</section><section className="drct-failure-cases"><header><div><h5>{selectedCondition ? <><b>{selectedCondition}</b> · {mismatch.conditions.find(item => item.code === selectedCondition)?.label}</> : "전체 불일치 사례"}</h5><strong>{caseRows.length}건</strong></div><p>{selectedCondition ? `${selectedCondition} 조건을 만족하지 못한 Marker 사례입니다.` : "검색조건을 만족하지 못한 Marker 사례입니다."}</p></header><CaseTable rows={caseRows} onOpen={openCase} /></section></div></> : <><div className="drct-tab-title"><div><h4>{category === "incomplete" ? "Rule 데이터 부족" : category === "undecided" ? "미판정 사례" : "모델 불일치 사례"}</h4><p>{category === "incomplete" ? "실제 진단에 기록된 데이터 부족 사유를 확인합니다." : category === "undecided" ? "Marker D0가 명확한 추가 복기 대상입니다." : "기본모델이 준비된 경우에만 표시합니다."}</p></div></div><CaseTable rows={caseRows} onOpen={openCase} /></>}
    </div> : null}
    ...
    {caseDetail ? <DiagnosticModal title={caseDetail.stock_name} subtitle={caseDetail.stock_code} meta={`Marker D0 ${caseDetail.d0} · 복기 ${koreanLabel(caseDetail.label)} · ${caseDetail.matched_marker_names.join(", ")}`} rows={caseDetail.rule_diagnostics as Array<{code:string;label:string;status:string;actual_value:string;criteria:string}>} onClose={() => setCaseDetail(null)} /> : null}
  </section>;
}
...
function CaseTable({ rows, onOpen }: { rows: DrctTrainingCase[]; onOpen: (item: DrctTrainingCase) => void }) {
  ...
  return <div className="drct-case-table"><table><thead><tr><th>종목</th><th>Marker D0</th><th>복기</th><th>Marker</th><th>실패 조건</th><th>상세</th></tr></thead><tbody>{rows.map((item) => { const failed=item.failed_conditions??[]; return <tr key={`${item.stock_id}-${item.d0}`} tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onOpen(item); }} onClick={() => onOpen(item)}><td title={item.stock_name}><strong>{item.stock_name}</strong><small>{item.stock_code}</small></td><td>{item.d0.replace(/-/g, ".")}</td><td><em className={`drct-case-outcome ${item.label.toLowerCase()}`}>{koreanLabel(item.label)}</em></td><td title={item.matched_marker_names.join(", ")}>{item.matched_marker_names.join(", ").replace(" - ", " · ")}</td><td><div className="drct-failed-chip-strip" title={failed.map(row=>`${row.code} ${row.label}`).join(", ")}>{failed.slice(0,4).map((row) => <span key={row.code}>{row.code}</span>)}{failed.length>4?<small>+{failed.length-4}</small>:null}</div></td><td><button type="button" className="btn btn-secondary btn-table-sm">보기</button></td></tr>;})}</tbody></table></div>;
}

const DIAGNOSTIC_VALUE_LABELS: Record<string,string[]> = {
  C:["20일 이동평균선","60일 이동평균선","120일 이동평균선"], D:["현재 60일선","1봉 전","2봉 전"],
  E:["현재 종가","5봉 전 종가","8봉 전 고가"], F:["종가","5일 이동평균선"],
  H:["현재 종가","7봉 전 종가","12봉 전 고가"], I:["종가","10일 이동평균선"],
  J:["현재 종가","10봉 전 종가","20봉 전 고가"], K:["종가","20일 이동평균선"],
  L:["현재 240일선","1봉 전","2봉 전"],
};
function diagnosticValues(code:string,actual:string){
  const values=(actual.match(/[-+]?\d{1,3}(?:,\d{3})*(?:\.\d+)?|[-+]?\d+(?:\.\d+)?/g)??[]).filter((value,index,list)=>index===0||value!==list[index-1]);
  const labels=DIAGNOSTIC_VALUE_LABELS[code]??[];
  return values.map((value,index)=>({label:labels[index]??`값 ${index+1}`,value}));
}

function diagnosticDifference(code:string,values:Array<{label:string;value:string}>){
  if (!["F","I","K"].includes(code)||values.length<2) return null;
  const numbers=values.slice(0,2).map(item=>Number(item.value.replace(/,/g,"")));
  if (numbers.some(Number.isNaN)||numbers[1]===0) return null;
  const difference=numbers[0]-numbers[1];
  return `차이 ${difference.toLocaleString("ko-KR")}원 · ${(difference/numbers[1]*100).toFixed(2)}%`;
}

function DiagnosticModal({ title, subtitle, meta, rows, onClose }: { title: string; subtitle: string; meta?: string; rows: Array<{code:string;label:string;status:string;actual_value:string;criteria:string}>; onClose: () => void }) {
  const [filter,setFilter]=useState<"FAIL"|"ALL"|"PASS"|"DATA_INCOMPLETE">("FAIL");
  const counts=(status:string)=>rows.filter(row=>row.status===status).length;
  const visible=filter==="ALL"?rows:rows.filter(row=>row.status===filter);
  return <div className="drct-signal-modal-backdrop" role="presentation" onMouseDown={(event)=>event.target===event.currentTarget&&onClose()}><section className="drct-signal-modal drct-rule-diagnostic-modal" role="dialog" aria-modal="true" aria-label="Marker D0 검색조건 진단"><header><div><small>Marker D0 검색조건 진단</small><h3>{title} <span>{subtitle}</span></h3><p>{meta || subtitle}</p></div><button type="button" aria-label="닫기" onClick={onClose}><X size={19} /></button></header><section className="drct-diagnostic-summary"><strong>검색조건 불일치</strong><div><span>충족 <b>{counts("PASS")}</b></span><span>미충족 <b>{counts("FAIL")}</b></span><span>데이터 부족 <b>{counts("DATA_INCOMPLETE")}</b></span></div><p>실패 조건 {rows.filter(row=>row.status==="FAIL").map(row=><b key={row.code}>{row.code}</b>)}</p></section><nav className="drct-diagnostic-filters">{([['FAIL',`미충족 ${counts("FAIL")}`],['ALL',`전체 ${rows.length}`],['PASS',`충족 ${counts("PASS")}`],['DATA_INCOMPLETE',`데이터 부족 ${counts("DATA_INCOMPLETE")}`]] as const).map(([key,label])=><button key={key} className={filter===key?"is-active":""} onClick={()=>setFilter(key)}>{label}</button>)}</nav><div className="drct-rule-diagnostic-list">{visible.map((row)=>{const values=diagnosticValues(row.code,row.actual_value||"");const difference=diagnosticDifference(row.code,values);return <article key={row.code} className={`status-${row.status.toLowerCase()}`}><span>{row.code}</span><div><strong>{row.label}</strong><dl>{values.length?values.map(item=><div key={item.label}><dt>{item.label.replace(" 이동평균선","선")}</dt><dd>{item.value}{["F","I","K"].includes(row.code)?"원":""}</dd></div>):<div><dt>실제 값</dt><dd>-</dd></div>}{difference?<p>{difference}</p>:null}</dl><details><summary>기술 상세 보기</summary><small>{row.criteria||"-"}</small></details></div><em>{resultLabel(row.status)}</em></article>})}</div><footer><p><Info size={14}/> 이 결과는 Marker D0 시점의 데이터로 검색조건을 다시 평가한 결과입니다.</p><button type="button" className="btn btn-secondary" onClick={onClose}>닫기</button></footer></section></div>;
}
