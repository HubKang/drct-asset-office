...
import type { DrctSignalMarkerOptionGroup, DrctSignalSearch, DrctSignalSearchCreate, DrctSignalSearchDetail, DrctSignalVersion, DrctSignalVersionCreate } from "@/types/drctStockSignal";
...
function SignalResearchTab() {
  ...
  const loadSearches = useCallback(async (preferredId?: number) => {
    ...
    try {
      const rows = await repositories.drctStockSignals.listSearches(true);
      setSearches(rows);
  ...
  return <div className="drct-signal-research-layout phase-two">
    <section className="drct-signal-panel drct-signal-research-list" aria-labelledby="condition-list-title">
      ...
      {!loading && searches.length === 0 ? <SignalEmptyState icon={BookOpen} title="등록된 DrCT 검색식이 없습니다." description="새 검색식을 등록해 HTS 참조 조건과 Version을 관리하세요." compact /> : <div className="drct-signal-search-list">{searches.map((search) => <button key={search.id} type="button" className={selectedId === search.id ? "is-selected" : ""} onClick={() => setSelectedId(search.id)}><span className="drct-signal-search-name"><i className={search.is_active ? "" : "is-inactive"} />{search.name}</span><span className="drct-signal-search-meta"><b>{search.lifecycle_status}</b><em>v{search.current_version_no}</em></span><span className="drct-signal-search-counts">관련 차트마커 {search.training_summary.linked_marker_count}개</span><ChevronRight size={15} /></button>)}</div>}
    </section>

    <section className="drct-signal-panel drct-signal-research-detail" aria-labelledby="condition-detail-title">
      {!detail ? <SignalEmptyState icon={FlaskConical} title="검색식을 선택하면 정의와 Version을 확인할 수 있습니다." description="HTS 참조식과 DrCT 실행식은 서로 구분해 관리합니다." /> : <div className="drct-signal-detail-content">
        <header className="drct-signal-detail-header phase5c"><div><div className="drct-definition-badges"><span className={`drct-signal-status ${detail.lifecycle_status.toLowerCase()}`}>{detail.lifecycle_status}</span><span>v{detail.current_version_no}</span><span className={detail.current_version.structured_rule?.validation_status === "VALID" ? "is-ready" : "is-pending"}>{detail.current_version.structured_rule?.validation_status === "VALID" ? "검색조건 완료" : "검색조건 변환 필요"}</span></div><h2 id="condition-detail-title">{detail.name}</h2><p>{detail.description || "검색식 설명이 없습니다."}</p></div><div className="drct-signal-detail-actions"><button type="button" className="btn btn-secondary" onClick={() => void openMarkers()}>관련 마커</button><button type="button" className="btn btn-secondary" onClick={openEdit}><Pencil size={14} /> 수정</button><button type="button" className="btn btn-secondary" onClick={() => void openHistory()}><History size={14} /> 버전 이력</button><button type="button" className={`btn btn-secondary ${detail.is_active ? "danger" : ""}`} disabled={saving} onClick={() => void setActive(!detail.is_active)}><Power size={14} /> {detail.is_active ? "비활성" : "활성화"}</button></div></header>
    ...
    {modal ? <div className="drct-signal-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) setModal(null); }}><section className={`drct-signal-modal ${modal === "history" || modal === "markers" || modal === "source" ? "is-wide" : ""}${modal === "markers" ? " is-marker-picker" : ""}`} role="dialog" aria-modal="true" aria-label="검색식 관리"><header><div><h3>{modal === "create" ? "새 검색식" : modal === "edit" ? "검색식 기본정보 수정" : modal === "version" ? `새 Version · v${(detail?.current_version_no ?? 0) + 1}` : modal === "markers" ? "관련 차트마커 관리" : modal === "source" ? "HTS 전체 조건" : "Version History"}</h3><p>{modal === "version" ? "기존 Version은 보존되고 새 Version이 Current가 됩니다." : modal === "markers" ? "검색식과 Marker의 참고 관계만 변경합니다." : modal === "source" ? "읽기 전용 HTS 참조 원문과 최종 조건식입니다." : "DrCT 종목별 마크 시그널 검색식 관리"}</p></div><button type="button" aria-label="닫기" disabled={saving} onClick={() => setModal(null)}><X size={19} /></button></header>
      ...
      {modal === "markers" ? <div className="drct-signal-marker-picker"><p className="drct-reference-helper">관련 차트마커는 검색식 학습의 Gate가 아니라 향후 판별력 비교를 위한 참고 관계입니다.</p>{markerOptions.length ? markerOptions.map((group) => <fieldset key={group.id}><legend><i style={{ background: group.color }} />{group.name}</legend>{group.markers.map((marker) => <label key={marker.id}><input type="checkbox" checked={checkedMarkerIds.includes(marker.id)} onChange={(e) => setCheckedMarkerIds((ids) => e.target.checked ? [...ids, marker.id] : ids.filter((id) => id !== marker.id))} /><span>{marker.symbol}</span><div><strong>{marker.name}</strong>{marker.description ? <small>{marker.description}</small> : null}</div></label>)}</fieldset>) : <p className="drct-signal-section-empty">관련지을 수 있는 활성 차트마커가 없습니다.</p>}<footer><button type="button" className="btn btn-secondary" onClick={() => setModal(null)}>취소</button><button type="button" className="btn btn-primary" disabled={saving} onClick={() => void submitMarkers()}>참고 관계 저장</button></footer></div> : null}
