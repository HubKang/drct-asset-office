...
function StockSignalTab() {
  ...
  const load=useCallback(async()=>{const request=++requestId.current;setBusy(true);setError("");try{const next=await loadScan();if(request===requestId.current){setScan(next);setDiagnostics(null);}}catch{if(request===requestId.current)setError("종목별 마크 시그널을 계산하지 못했습니다.");}finally{if(request===requestId.current)setBusy(false);}},[]);
