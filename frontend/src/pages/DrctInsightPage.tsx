...
type DrawerTab = "summary" | "pattern" | "flow";
...
function themeSignalSummary(theme: DrctInsightTheme, realtimeReady: boolean) {
  if (!realtimeReady || theme.theme_strength == null || !theme.linked_stock_count) return `${themeCompactLead(theme)} · 장중 데이터 대기`;
  return `${themeCompactLead(theme)} · 강도 ${point(theme.theme_strength, 2)} · 확산 ${theme.valid_stock_count}/${theme.linked_stock_count}`;
}
...
function PostMarketPanel({ data, views, filter, onFilter, onOpen }: { data: DrctInsightToday; views: Record<number, IntradayView>; filter: ReviewFilter; onFilter: (value: ReviewFilter) => void; onOpen: (stockId: number) => void }) {
  ...
  const reviews = data.review_queue.filter((item) => { const categories = reviewCategories(item, views[item.stock_id]); if (filter === "ALL") return true; if (filter === "UNRECORDED") return item.outcome.marker_status === "UNRECORDED"; return categories.includes(filter); }).slice(0, 10);
  return <section className="insight-ux-post"><header><div><small>장후 판단</small><h2>오늘 집중 후보 {summary.focus_count}개 중 {summary.positive_count}개가 상승했습니다.</h2><p>테마보다 강했던 종목 {summary.theme_outperform_count}개 · 복기 대상 {summary.review_count}개</p></div><dl><div><dt>평균 D0</dt><dd>{pct(summary.average_d0_return, 2)}</dd></div><div><dt>복기 추천</dt><dd>{summary.review_count}</dd></div></dl></header><nav aria-label="복기 필터">{([['ALL','전체'],['STRONG','강세'],['WEAK','약세'],['MISMATCH','판단 불일치'],['UNRECORDED','미기록']] as const).map(([key,label]) => <button type="button" key={key} className={filter === key ? "is-active" : ""} onClick={() => onFilter(key)}>{label}</button>)}</nav><div>{reviews.map((item) => <button type="button" key={item.stock_id} onClick={() => onOpen(item.stock_id)}><span>{item.theme_name || "연결 테마 없음"}</span><strong>{item.stock_name}</strong><em>{item.outcome.status === "NOT_READY" ? "평가 대기" : pct(item.outcome.d0_return, 2)}</em></button>)}</div></section>;
}
...
function InsightDrawer({ stock, view, postMarket, analysisDate, realtimeStatus, loading, tab, onTab, onRefresh, onClose }: { stock: DrctInsightStock; view: IntradayView; postMarket: boolean; analysisDate: string | null; realtimeStatus: ReadinessStatus; loading: boolean; tab: DrawerTab; onTab: (tab: DrawerTab) => void; onRefresh: () => void; onClose: () => void }) {
  ...
  const why = [stock.focus_candidate ? `오늘 집중 후보 #${stock.focus_rank}` : candidateLabel(stock), stock.observation_rank ? `${stock.theme_name || "연결 테마"} 관찰순위 #${stock.observation_rank}` : null, `성공 사례 유사도 ${number(stock.success_similarity)}`, stock.gates.flow === "PASS" ? "테마 수급 양호" : null].filter(Boolean) as string[];
...
function DrctInsightPage() {
  ...
  const [workspace, setWorkspace] = useState<"today" | "performance">("today"); const [themeFilter, setThemeFilter] = useState<ThemeFilter>("FOCUS"); const [stockFilter, setStockFilter] = useState<StockFilter>("FOCUS"); const [themeId, setThemeId] = useState<number | null>(null); const [reviewFilter, setReviewFilter] = useState<ReviewFilter>("ALL"); const [selected, setSelected] = useState<DrctInsightStock | null>(null); const [watchOpen, setWatchOpen] = useState(false); const [drawerTab, setDrawerTab] = useState<DrawerTab>("summary"); const [views, setViews] = useState<Record<number, IntradayView>>({});
  ...
  const toggleWatch = async (stock: DrctInsightStock) => { const watch = watches.find((row) => row.stock_id === stock.stock_id); if (watch) await repositories.watchlist.update(watch.watchlist_id, { is_active: 0 }); else await repositories.watchlist.bulkAdd({ stock_ids: [stock.stock_id], memo: "DrCT 오늘의 인사이트" }); await load(true); };
  ...
  if (workspace === "performance") return <div className="drct-insight-page"><PageHeader title="DrCT 오늘의 인사이트" description="실제 저장된 집중·검증 후보의 D+N 성과를 확인합니다."/><InsightPerformanceView onToday={() => setWorkspace("today")}/></div>;
  return <div className="drct-insight-page insight-ux-page">
    <header className="insight-ux-header"><div className="insight-ux-title"><div><span>장중 투자 작업대</span><h1>DrCT 오늘의 인사이트</h1></div><div className="insight-ux-title-actions"><button type="button" className="insight-watch-button" onClick={() => setWatchOpen(true)}><Star size={16} fill={watches.length ? "currentColor" : "none"}/>내 관찰 <b>{watches.length}</b></button><button type="button" className="btn btn-secondary" disabled={loading} onClick={() => void load()}><RefreshCw size={15}/>{loading ? "갱신 중" : "새로고침"}</button></div></div><div className="insight-ux-judgment"><span>오늘의 판단</span><p>{judgment}</p></div><div className="insight-ux-header-bottom"><nav aria-label="DrCT 오늘의 인사이트 화면 전환"><button type="button" className="is-active">오늘의 판단</button><button type="button" onClick={() => setWorkspace("performance")}>후보 성과</button></nav><dl><div><dt>관찰 테마</dt><dd>{data?.summary.observed_theme_count ?? "-"}</dd></div><div><dt>집중 후보</dt><dd>{data?.summary.focus_candidate_count ?? "-"}</dd></div><div><dt>검증 후보</dt><dd>{data?.summary.final_candidate_count ?? "-"}</dd></div><div><dt>실행 검토</dt><dd>{data?.summary.ready_count ?? "-"}</dd></div></dl><div className="insight-ux-meta">{data ? <Readiness data={data}/> : null}<span>{data?.market_mode === "POST_MARKET" ? "장후" : data?.market_mode === "PRE_MARKET" ? "장전" : "장중"} · 마지막 {data?.summary.last_updated_at?.slice(11,16) || "-"}</span></div></div></header>
