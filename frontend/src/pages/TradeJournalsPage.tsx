...
function TradeJournalsPage() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="매매일지" description="목록/상세 관리 및 GPT 복기 패키지 생성" />
      <SectionCard title="검색">
        <div className="grid grid-cols-1 gap-2 md:grid-cols-5">
          ...
          <input className="input-control" value={filters.stock_name} onChange={(e) => setFilters((p) => ({ ...p, stock_name: e.target.value }))} placeholder="종목명" />
          ...
          <button type="button" className="btn btn-primary" onClick={() => void loadList()}>{loading ? "조회중" : "조회"}</button>
        </div>
      </SectionCard>
      ...
      <SectionCard title="목록">
        <div className="mb-2 flex justify-end"><button type="button" className="btn btn-primary" onClick={openCreate}>추가</button></div>
        <div className="trade-journal-table-shell">
          <table className="data-table trade-journal-table">
            <thead><tr><th>매수일</th><th>종목명</th><th>상태</th><th>수익률</th><th>실현손익</th><th>이미지</th></tr></thead>
            <tbody>
              {items.map((item) => (
              ...
              <tr className="trade-journal-summary-row"><td colSpan={4}>실현손익 합계</td><td>{formatWon(totalRealized)}</td><td /></tr>
            </tbody>
          </table>
        </div>
      </SectionCard>
          ...
          <aside className="trade-journal-detail-drawer">
            <div className="trade-journal-detail-drawer-header">
              ...
              <div className="trade-detail-header-actions">
                <button className="btn btn-secondary" onClick={() => void generatePackage()} type="button">GPT 복기 패키지 생성</button>
                <button className="btn btn-secondary" onClick={() => void copyPackage()} type="button" disabled={!gptPackage}>GPT 분석 요청문+JSON 복사</button>
                <button className="btn btn-secondary" onClick={() => setIsDetailOpen(false)} type="button">닫기</button>
              </div>
            </div>
            <div className="trade-journal-detail-drawer-body">
              <div className="trade-detail-form-grid">
                <div className="trade-detail-field"><label>매수일</label><input className="input-control" type="date" value={(form.buy_date || "").slice(0,10)} onChange={(e) => setForm((p) => ({ ...p, buy_date: e.target.value }))} /></div>
                <div className="trade-detail-field"><label>매도일</label><input className="input-control" type="date" value={(form.sell_date || "").slice(0,10)} onChange={(e) => setForm((p) => ({ ...p, sell_date: e.target.value }))} /></div>
                <div className="trade-detail-field"><label>종목명</label><input className="input-control" value={form.stock_name || ""} onChange={(e) => setForm((p) => ({ ...p, stock_name: e.target.value }))} /></div>
                <div className="trade-detail-field"><label>테마</label><input className="input-control" value={form.stock_theme || ""} onChange={(e) => setForm((p) => ({ ...p, stock_theme: e.target.value }))} /></div>
                <div className="trade-detail-field"><label>매매기법</label><select className="select-control" value={String(form.trade_method_id ?? "")} onChange={(e) => setForm((p) => ({ ...p, trade_method_id: e.target.value ? Number(e.target.value) : null }))}><option value="">선택</option>{tradeMethods.map((m) => <option key={m.id} value={m.id}>{m.method_name}</option>)}</select></div>
                <div className="trade-detail-field"><label>상태</label><select className="select-control" value={form.result_type || "holding"} onChange={(e) => setForm((p) => ({ ...p, result_type: e.target.value }))}>{RESULT_TYPE_OPTIONS.filter((x) => x.value).map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}</select></div>
              </div>

              <div className="detail-section">
                <textarea className="textarea-control" placeholder="매매기록" value={form.remark || ""} onChange={(e) => setForm((p) => ({ ...p, remark: e.target.value }))} />
              </div>

              {gptPackage ? (
                <div className="detail-section">
                  <div className="gpt-package-preview">
                    <div className="gpt-package-preview-header"><strong>GPT 복기 패키지 미리보기</strong><button type="button" className="btn btn-secondary btn-table-sm" onClick={() => void copyPackage()}>전체 복사</button></div>
              ...
              <div className="detail-section">
                <div className="mb-2 flex items-center justify-between"><strong>차트 이미지</strong></div>
                ...
                <div className="mt-2"><button type="button" className="btn btn-secondary" onClick={() => void uploadImage()} disabled={!selectedJournalId || !imageFile}>이미지 업로드</button></div>
            ...
            <div className="trade-journal-detail-drawer-footer">
              <button className="btn btn-primary" type="button" onClick={() => void save()} disabled={saving}>{saving ? "저장중" : "저장"}</button>
            </div>
          </aside>
        </>
      ) : null}
