...
function ReviewChart({ data, event, loading }: { data: ChartMarkerReviewChart | null; event: ChartMarkerReviewEvent; loading: boolean }) {
  ...
  const sourceRows = data.candles;
  const sourceD0 = data.marker_index != null && sourceRows[data.marker_index]?.trade_date === data.marker_date
    ? data.marker_index
    : sourceRows.findIndex((row) => row.trade_date === data.marker_date);
  if (sourceD0 < 0) return <div className="chart-marker-empty">마커 기준일의 가격 데이터를 찾을 수 없습니다.</div>;
  const sliceStart = Math.max(0, sourceD0 - SIDE_CANDLE_COUNT);
  const rows = sourceRows.slice(sliceStart, sourceD0 + SIDE_CANDLE_COUNT + 1);
  const d0 = sourceD0 - sliceStart;
  ...
  const slotCount = SIDE_CANDLE_COUNT * 2 + 1;
  const x = (index: number) => p.l + (SIDE_CANDLE_COUNT - d0 + index + .5) * (width - p.l - p.r) / slotCount;
  ...
  return <div className="chart-marker-review-chart">
    ...
    <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${event.stock_name} ${data.marker_date} 전후 차트`} onMouseLeave={() => setHovered(null)}>
...
export default function ChartMarkerReviewPage() {
  ...
  useEffect(() => {
    ...
    repositories.chartMarkers.reviewChart(selected.stock_id, selected.marker_date)
      .then((nextChart) => { if (active) setChart(nextChart); })
