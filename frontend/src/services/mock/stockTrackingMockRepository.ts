  ...
  registerFromConditionResults: async (payload: CreateTrackingFromConditionResultsPayload) => {
    ...
    const resultItems = payload.items.map((source) => {
      ...
      const row: StockTrackingItem = {
        ...
        entry_close_price: source.current_price ?? null,
        entry_close_date: payload.detected_date,
        latest_close_price: source.current_price ?? null,
        latest_close_date: payload.detected_date,
        tracking_return_pct: null,
