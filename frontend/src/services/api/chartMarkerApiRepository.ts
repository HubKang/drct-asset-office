...
export const chartMarkerApiRepository = {
  ...
  deleteMarker: (id:number) => apiRequest<{deleted:boolean;id:number}>(`/chart-markers/markers/${id}`, {method:"DELETE"}),
  ...
  updateEvent: (id:number,payload:{marker_id?:number;memo?:string|null;review_result?:ChartMarkerReviewResult}) => apiRequest<ChartMarkerEvent>(`/chart-markers/events/${id}`,{method:"PATCH",body:JSON.stringify(payload)}),
  ...
  reviewChart: (stockId:number,markerDate:string,beforeCandles=60,afterCandles=20) => apiRequest<ChartMarkerReviewChart>(`/chart-markers/review/chart?stock_id=${stockId}&marker_date=${markerDate}&before_candles=${beforeCandles}&after_candles=${afterCandles}&candle_count=141`),
};
